...
// Copyright (c) 2014-2014 The AudioCoin developers
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // AudioCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // AudioCoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // AudioCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// AudioCoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // AudioCoin: subsidy is cut in half every 16x multiply of difficulty
    ...
    // setting the reward per block
    int64 nSubsidy; // = bnUpperBound.getuint64();
    // nSubsidy = (nSubsidy / CENT) * CENT;
    nSubsidy = MAX_MONEY / 2; //max number of coins / 2 (2 blocks will contain 100% of the coins)
...
// AudioCoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// AudioCoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // AudioCoin: target change every block
    // AudioCoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // AudioCoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            // AudioCoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // AudioCoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // AudioCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // AudioCoin: track money supply and mint amount info
    ...
    // AudioCoin: fees are not collected by miners as in bitcoin
    // AudioCoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// AudioCoin: total coin age spent in transaction, in the unit of coin-days.
...
// AudioCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // AudioCoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // AudioCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // AudioCoin: record proof-of-stake hash value
    ...
    // AudioCoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // AudioCoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // AudioCoin: coinbase output should be empty if proof-of-stake block
    ...
    // AudioCoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // AudioCoin: check that the block satisfies synchronized checkpoint
    ...
    // AudioCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // AudioCoin: check proof-of-stake
    ...
    // AudioCoin: verify hash target and signature of coinstake tx
    ...
    // AudioCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // AudioCoin: getblocks may not obtain the ancestor block rejected
    ...
    // AudioCoin: if responsible for sync-checkpoint send it
...
// AudioCoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "AudioCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "AudioCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        /*
         *  If set to true, the genesis block will be generated and mined.
         *  The information about the genesis block will be printed to the
         *  console and the log file.
         */
        bool findGenesisBlock = false;
        ...
        if (findGenesisBlock) {
            ...
            //Thank you, sunny. https://github.com/sunnyking
        ...
        // AudioCoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // AudioCoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // AudioCoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // AudioCoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // AudioCoin: should not enter safe mode for longer invalid chain
    // AudioCoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // AudioCoin: if detected invalid checkpoint enter safe mode
    ...
    // AudioCoin: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // AudioCoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // AudioCoin: record my external IP reported by peer
        ...
        // AudioCoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // AudioCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // AudioCoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // AudioCoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // AudioCoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // AudioCoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
