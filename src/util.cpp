...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows: C:\Documents and Settings\username\Application Data\AudioCoin
    // Mac: ~/Library/Application Support/AudioCoin
    // Unix: ~/.AudioCoin
#ifdef WIN32
    // Windows
    return MyGetSpecialFolderPath(CSIDL_APPDATA, true) / "AudioCoin";
#else
    fs::path pathRet;
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("AudioCoin"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) / "AudioCoin.lnk";
}
