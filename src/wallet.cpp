...
// AudioCoin: optional setting to unlock wallet for block minting only;
...
// AudioCoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // AudioCoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // AudioCoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // AudioCoin: not avatar mode
                    {
...
// AudioCoin: create coin stake transaction
...
// AudioCoin: check 'spent' consistency between wallet and txindex
// AudioCoin: fix wallet spent state according to txindex
...
// AudioCoin: disable transaction (only for coinstake)
